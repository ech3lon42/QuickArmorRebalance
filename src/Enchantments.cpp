...
bool QuickArmorRebalance::ApplyEnhancedEnchantment(RE::TESBoundObject* item, const EnhancedItemConfig& config) {
    if (!item || !config.HasEnchantment()) {
        return false;
    }

    // Look up the enchantment from FormID string
    auto enchForm = LookupForm(config.enchantmentFormID);
    if (!enchForm) {
        logger::warn("ApplyEnhancedEnchantment: Could not find enchantment {}", config.enchantmentFormID);
        return false;
    }

    auto enchantment = enchForm->As<RE::EnchantmentItem>();
    if (!enchantment) {
        logger::warn("ApplyEnhancedEnchantment: Form {} is not an enchantment", config.enchantmentFormID);
        return false;
    }
    ...
    // We need to find the item in the player's inventory and add the enchantment to it
    // Since GetInventory() can cause issues with other plugins, we use a task-based approach
    SKSE::GetTaskInterface()->AddTask([=]() {
        auto playerRef = RE::PlayerCharacter::GetSingleton();
        if (!playerRef) return;

        // Get inventory - this runs in a task so should be safer
        auto inv = playerRef->GetInventory();
        auto it = inv.find(item);
        if (it == inv.end()) {
            logger::warn("ApplyEnhancedEnchantment: Item {} not found in inventory", item->GetName());
            return;
        }

        auto& entry = it->second;
        if (!entry.second || !entry.second->extraLists || entry.second->extraLists->empty()) {
            logger::warn("ApplyEnhancedEnchantment: No extra data lists for {}", item->GetName());
            return;
        }

        // Get the first extra data list and add the enchantment
        for (auto* extraList : *entry.second->extraLists) {
            if (!extraList) continue;

            // Check if already has an enchantment
            if (extraList->HasType(RE::ExtraEnchantment::EXTRADATATYPE)) {
                // Already enchanted, skip
                continue;
            }

            // Add the enchantment (charge is 0 for armor, which means infinite)
            extraList->Add(new RE::ExtraEnchantment(enchantment, 0));
            logger::trace("ApplyEnhancedEnchantment: Applied {} to {}",
                enchantment->GetName(), item->GetName());
            return;
        }
    });
