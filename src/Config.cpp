...
bool QuickArmorRebalance::Config::Load() {
    ...
    // UI configs
    {
        ...
        if (config) {
            ...
            g_Config.nFontSize = std::clamp(config["settings"]["fontsize"].value_or(13), kFontSizeMin, kFontSizeMax);
            ...
            g_Config.outfitCycleTimeout = config["settings"]["outfitcycletimeout"].value_or(800);
            g_Config.outfitFrameMultiplier = config["settings"]["outfitframemultiplier"].value_or(2.0f);
            g_Config.itemListLimit = config["settings"]["itemlistlimit"].value_or(2000);
            ...
            if (auto tblCols = config["tableColumns"].as_table()) {
                if (auto arrVisible = (*tblCols)["visible"].as_array()) {
                    int idx = 0;
                    arrVisible->for_each([&idx](auto v) {
                        if (idx < Config::TableColumnConfig::Col_Count) {
                            if (auto b = v.as_boolean())
                                g_Config.tableColumns.visible[idx] = b->get();
                            idx++;
                        }
                    });
                }
                if (auto arrOrder = (*tblCols)["order"].as_array()) {
                    int idx = 0;
                    arrOrder->for_each([&idx](auto v) {
                        if (idx < Config::TableColumnConfig::Col_Count) {
                            if (auto i = v.as_integer())
                                g_Config.tableColumns.order[idx] = static_cast<int>(i->get());
                            idx++;
                        }
                    });
                }
            ...
            if (auto arr = config["settings"]["itemblacklist"].as_array()) {
                g_Config.itemBlacklist.reserve(arr->size());
                arr->for_each([this](auto i) {
                    if (auto str = i.as_string())
                        if (!str->get().empty()) g_Config.itemBlacklist.push_back(MakeLower(str->get()));
                });
            }
...
void QuickArmorRebalance::Config::Save() {
    ...
    auto tomlItemBlacklist = toml::array{};
    tomlItemBlacklist.insert(tomlItemBlacklist.begin(), itemBlacklist.begin(), itemBlacklist.end());
    ...
    auto tbl = toml::table{
        ...
        {"settings", toml::table{{"fontsize", g_Config.nFontSize},
                                 ...
                                 {"outfitcycletimeout", g_Config.outfitCycleTimeout},
                                 {"outfitframemultiplier", g_Config.outfitFrameMultiplier},
                                 {"itemlistlimit", g_Config.itemListLimit},
                                 {"allrecipereqs", g_Config.bShowAllRecipeConditions},
                                 {"autodisablewords", tomlDisableWords},
                                 {"itemblacklist", tomlItemBlacklist},
        ...
        {"tableColumns", toml::table{
            {"visible", toml::array{
                g_Config.tableColumns.visible[0],
                g_Config.tableColumns.visible[1],
                g_Config.tableColumns.visible[2],
                g_Config.tableColumns.visible[3],
                g_Config.tableColumns.visible[4],
                g_Config.tableColumns.visible[5]
            }},
            {"order", toml::array{
                g_Config.tableColumns.order[0],
                g_Config.tableColumns.order[1],
                g_Config.tableColumns.order[2],
                g_Config.tableColumns.order[3],
                g_Config.tableColumns.order[4],
                g_Config.tableColumns.order[5]
            }}
        }},
    };
