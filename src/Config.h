...
namespace QuickArmorRebalance {
    ...
    // Bounds for Config::nFontSize
    const int kFontSizeMin = 10;
    const int kFontSizeMax = 32;
    ...
    struct Config {
        ...
        std::vector<std::string> itemBlacklist;  // Item name filter strings (one per line, case-insensitive)
        ...
        int outfitCycleTimeout = 800;  // Timeout in milliseconds for outfit cycling (Numpad+/-)
        float outfitFrameMultiplier = 2.0f;  // Frame skip multiplier for outfit operations (provides buffer for slower machines)
        int itemListLimit = 2000;  // Maximum number of items to display in the general items list
        ...
        // Table column configuration for items tables
        struct TableColumnConfig {
            enum Column : int {
                ...
                Col_Armor = 2,
                Col_ID = 3,
                Col_NifF = 4,  // Female NIF model path
                Col_Mod = 5,   // Source mod filename
                Col_Count = 6
            };

            bool visible[Col_Count] = {true, true, true, true, false, false};  // Default: NIF and Mod hidden
            int order[Col_Count] = {0, 1, 2, 3, 4, 5};  // Default order: Name, Slot, Armor, ID, NifF, Mod

            // Cached values (mutable for lazy evaluation)
            mutable int cachedDisplayOrder[Col_Count] = {0, 1, 2, 3, 4, 5};
            mutable int cachedVisibleCount = 4;
            mutable bool cacheValid = false;

            // Get column name for display
            static const char* GetColumnName(int col) {
                static const char* names[] = {"Name", "Slot", "Armor", "ID", "Nif (F)", "Mod"};
                return (col >= 0 && col < Col_Count) ? names[col] : "";
            }

            // Rebuild cache when config changes
            void RebuildCache() const {
                // Build display order
                std::pair<int, int> orderPairs[Col_Count];
                for (int i = 0; i < Col_Count; i++) {
                    orderPairs[i] = {order[i], i};
                }
                std::sort(orderPairs, orderPairs + Col_Count);
                for (int i = 0; i < Col_Count; i++) {
                    cachedDisplayOrder[i] = orderPairs[i].second;
                }

                // Count visible columns
                cachedVisibleCount = 0;
                for (int i = 0; i < Col_Count; i++) {
                    if (visible[i]) cachedVisibleCount++;
                }

                cacheValid = true;
            }

            // Get cached display order (pointer to array, no allocation)
            const int* GetDisplayOrder() const {
                if (!cacheValid) RebuildCache();
                return cachedDisplayOrder;
            }

            // Count visible columns (cached)
            int GetVisibleCount() const {
                if (!cacheValid) RebuildCache();
                return cachedVisibleCount;
            }

            // Invalidate cache (call when visible or order changes)
